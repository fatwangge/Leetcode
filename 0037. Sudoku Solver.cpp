...
public:
    ...
    bool solveSudokuHelper(vector<vector<char>>& board){
          for(int i = 0; i < 9 ; i++){
            for(int  j = 0; j < 9; j++){
                ...
                for(int k = 1; k <= 9; k++){
                    board[i][j] = '0' + k;
                    if(isValidSodoku(board, i, j) && solveSudokuHelper(board)) return true;
                    else board[i][j] = '.';
                }
                return false;
            }
    ...
    bool isValidSodoku(vector<vector<char>>& board, int i, int j) {//no need to validate all board, just board[i][j]
        for(int k = 0; k < 9; k++)
